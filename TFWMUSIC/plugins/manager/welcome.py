...
from TFWMUSIC.utils.TFW_ban import admin_filter
...
def welcomepic(pic_path, user, chatname, user_id, uname):
    background = Image.open("TFWMUSIC/assets/TFWwel2.png")
    pfp = Image.open(pic_path).convert("RGBA")
    pfp = circle(pfp, size=(835, 839))
    draw = ImageDraw.Draw(background)
    font_large = ImageFont.truetype('TFWMUSIC/assets/font2.ttf', size=65)
    ...
    if member.new_chat_member and member.new_chat_member.status == enums.ChatMemberStatus.MEMBER:
        try:
            ...
            if user.photo:
                ...
                pic_path = "TFWMUSIC/assets/wel2.png"
...
""",
            ...
            if pic_path and os.path.exists(pic_path) and "TFWMUSIC/assets/wel2.png" not in pic_path:
                os.remove(pic_path)
